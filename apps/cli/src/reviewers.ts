 ...
 *
 * Both modes produce a core `ReviewResult` (the reviewer's verdict), then run the
 * SAME assembly (`toReviewFnResult`): mint the persisted `Review` primitive
 * (findings get db ids) and build the two draft-first artifacts via the
 * reviewer's own `buildReviewDrafts`. Only the ReviewResult SOURCE differs:
 ...
 * warehouse (proves the seed→demo data flow); falls back to reading
 * `data/corpus/guidelines.json` directly so `review <file>` works even before a
 * `seed`. `scanDeterministic` filters by `type`/`category` itself, so passing
 * the full set (incl. approved_messaging rows) is fine.
 */
 ...
 * Exposed for the `review <file>` helper as well as the workflow reviewFn.
 */
