 ...
 *
 *   - offline: `resolveAccount` (context engine, no LLM) + `RulesScorer` → a
 *     TEMPLATED core `Decision` (relevantSignals = the account's REAL signalIds,
 *     committee from the enrichment record, a templated next-best-action) + a
 *     templated pending outreach `Draft`. No network, no LLM.
 *   - live: `@mstack/account-intel#activateAccount` (the SDR→Copywriter→Router
 *     swarm), ADAPTED — see `liveActivateFn` for the AccountDecision→Decision
 *     wrinkle.
 *
 * GUARDRAIL #6 ("never invent a signal"): offline `relevantSignals` cite only
 * ids that actually appear in the account's persisted `Signal[]`.
...
/** Most-recent-first, capped, real ids only — mirrors the SDR-Researcher
 *  surfacing the relevant subset (never inventing a signal). */
...
/**
 * OFFLINE activateFn — resolve (context engine) + RulesScorer, then TEMPLATE the
 * full `Decision` + a pending outreach `Draft`. No swarm, no LLM. Persists the
 * scored account onto its row (compounding memory), mirroring `activateAccount`.
 */
 ...
 *
 * WRINKLE (documented in workflows/account-activation.ts): `activateAccount`
 * returns the smaller `AccountDecision` BRIEF and self-persists its own full
 * `Decision`/`Draft`/`Account`. `runAccountActivation` wants the full `Decision`
 * primitive, so we reconstruct it from the brief — `accountId`/`ts` come from
 * the returned draft (`refId` = account.id, `createdAt` = the swarm timestamp).
 * `runAccountActivation` then persists this reconstructed row; because
 * `activateAccount` already wrote one under its own id, a live run leaves TWO
 * (equally valid) decision rows for the activation — a known, harmless
 * live-wiring artifact. The offline path (the make-or-break demo) never touches
 * this function.
 */
